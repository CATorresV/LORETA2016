function [J,AYYA] = spm_eeg_invert_red(Y,L,QG,vert,face,inverse)
...
%Temporal projector
[U E V]  = svd(Y,'econ');			% get temporal modes
E      = diag(E)/trace(E);			% normalise variance
Nr     = min(length(E),Nmax);			% number of temporal modes
Nr=max(Nr,1); %% use at least one mode

AY = A*Y;
V      = V(:,1:Nr);
if strcmp(inverse.dec,'rbf')
    dyy= squareform(pdist(Y'));
    d=0.5*median(squareform(dyy));
    dk = exp(-dyy.^2/(2*d^2));
    %dk = (Y'*Y).^d;
    [V,D]=eig(dk);
    [la,id]=sort(diag(D),'descend');
    cs=cumsum(la/sum(diag(D)));
    %cs = cumsum((diag(D)/sum(diag(D))));
    in = find(cs< 0.9);
    Nr= length(in)
    V=V(:,id);
    V=V(:,1:Nr)
...
if strcmp(inverse.type_cov,'lineal')
    AYYA = AY*AY'; %lineal
elseif strcmp(inverse.type_cov,'rbf')
    AYYAl = AY*AY';
    Day = squareform(pdist(AY,'mahalanobis'));
    sopt = median(squareform(Day));
    %s0 = iqr(squareform(Day));
    %[sopt,~,s0] = kScaleOptimization_info_alpha(Day,inverse.type_cov_alpha) ;
    AYYA = exp(-Day.^2/(2*sopt^2)).*AYYAl;
elseif strcmp(inverse.type_cov,'corr')
    
    
    AYYAl = AY*AY';
    Day = squareform(pdist(AY,'mahalanobis'));
    sopt = mean(squareform(Day));
    %s0 = iqr(squareform(Day));
    %[sopt,~,s0] = kScaleOptimization_info_alpha(Day,inverse.type_cov_alpha) ;
    AYYA2 = exp(-Day.^2/(2*sopt^2)).*AYYAl;
    
    AYYA = ((A_corrent(AY)+AYYA2)/2).*AYYAl;
