function [J,AYYA] = spm_eeg_invert_red_kpca(Y,L,QG,vert,face,inverse)
...
%         end
    ...
    Dy = pdist(Y');
    sig = 10*median(Dy);
    Kky = exp(-squareform(Dy).^2/(2*sig^2)) ;
    [V,E] = eig(Kky);
    
    [la,ind] = sort(diag(E)/sum(diag(E)),'descend');
    V = V(:,ind);
    Nr = find(cumsum(la)>0.95,1,'first');
else
    [U,E,V]  = svd(Y,'econ');			% get temporal modes
    E      = diag(E)/trace(E);			% normalise variance
    %Nr     = min(length(E),Nmax);			% number of temporal modes
    Nr = find(cumsum(E)>0.95,1,'first');
end
